...
class PaginatedAPIMixin(object):
    ...
    def to_collection_dict(query, page, per_page, endpoint, **kwargs):
        ...
        data = {
            'items': [dict(item) for item in resources.items],
            '_meta': {
                'page': page,
                'per_page': per_page,
            ...
            '_links': {
                'self': url_for(endpoint, _external=True, page=page, per_page=per_page,
                                **kwargs),
                'next': url_for(endpoint, _external=True, page=page + 1, per_page=per_page,
                                **kwargs) if resources.has_next else None,
                'prev': url_for(endpoint, _external=True, page=page - 1, per_page=per_page,
...
class Agenda(db.Model):
    ...
    agenda_id = db.Column(db.Integer, primary_key=True, server_default=db.FetchedValue())
    title = db.Column(db.String(128), nullable=False)
    sitting_id = db.Column(db.ForeignKey('public.sitting.sitting_id'))
    body = db.Column(db.Text)

    sitting = db.relationship('Sitting', primaryjoin='Agenda.sitting_id == Sitting.sitting_id', backref='agendas')
...
class Alfie(db.Model):
    ...
    head = db.relationship('Doc', primaryjoin='Alfie.head_id == Doc.doc_id', backref='doc_alfies')
    sitting = db.relationship('Sitting', primaryjoin='Alfie.sitting_id == Sitting.sitting_id', backref='sitting_alfies')
...
class Group(PaginatedAPIMixin, Principal):
    ...
    parent_group = db.relationship('Group', remote_side=[group_id], primaryjoin='Group.parent_group_id == Group.group_id', backref='groups')
...
class User(PaginatedAPIMixin, Principal):
    ...
    country = db.relationship('Country', primaryjoin='User.birth_country == Country.country_id', backref='country_users')
    country1 = db.relationship('Country', primaryjoin='User.birth_nationality == Country.country_id', backref='country1_users')
    country2 = db.relationship('Country', primaryjoin='User.current_nationality == Country.country_id', backref='country2_users')
...
class Session(db.Model):
    ...
    parliament = db.relationship('Parliament', primaryjoin='Session.parliament_id == Parliament.parliament_id', backref='sessions')
...
class UserGroupMembership(db.Model):
    ...
    group = db.relationship('Group', primaryjoin='UserGroupMembership.group_id == Group.group_id', backref='group_memberships')
    replaced = db.relationship('UserGroupMembership', uselist=False, remote_side=[membership_id], primaryjoin='UserGroupMembership.replaced_id == UserGroupMembership.membership_id', backref='user_group_memberships')
    user = db.relationship('User', primaryjoin='UserGroupMembership.user_id == User.user_id', backref='user_memberships')
